            ...
            <div className={`${filter.filter} ${filter.algorithmFilter}`}>
                ...
                <ul>
                    {algorithmList.map((item, idx) => (
                        <li className={`${algoFocus === item.id ? filter.focusIn : filter.focusOut}`}
                            onMouseOver={()=>{setAlgoFocus(item.id)}}
                            onMouseOut={()=>{setAlgoFocus(0)}}
                            key = {item.id}>
                            {item.name}
                        </li>
                    ))}
            ...
            <div className={`${filter.filter} ${filter.problemFilter}`}>
                ...
                <ul>
                    {problemList.map((item, idx) => (
                        <li className={`${probFocus === item.id ? filter.focusIn : filter.focusOut}`}
                            onMouseOver={()=>{setProbFocus(item.id)}}
                            onMouseOut={()=>{setProbFocus(0)}}
                            key = {item.id}>
                            {item.name}
                        </li>
                    ))}
